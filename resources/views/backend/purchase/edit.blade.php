                ...
                <div class="modal-body">
                    <form>
                        <div class="row modal-element">
                            ...
                            <div class="col-md-4 form-group">
                                <label>{{__('db.Prodcut Price')}}</label>
...
for(rowindex  =0; rowindex <= rownumber; rowindex++){
    ...
    profit_margin.push(parseFloat($('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.profit-margin').val()));
...
$("table.order-list").on("click", ".edit-product", function() {
    ...
    $('input[name="edit_product_price"]').val(row_product_price.toFixed({{$general_setting->decimal}}));
...
$(document).ready(function () {
    ...
    function calculatePrice() {
        ...
        $price.val(Number.isInteger(price) ? price : price.toFixed(2));
    }
    ...
    function calculateMargin() {
        ...
        if (cost > 0) {
            ...
            $margin.val(Number.isInteger(margin) ? margin : margin.toFixed(2));
        } else {
            $margin.val(0);
        }
    }
    ...
    $cost.on("input", function () {
        if ($margin.val()) calculatePrice();
    });

    $margin.on("input", function () {
        if ($cost.val()) calculatePrice();
    });

    $price.on("input", function () {
        if ($cost.val()) calculateMargin();
    });
});
